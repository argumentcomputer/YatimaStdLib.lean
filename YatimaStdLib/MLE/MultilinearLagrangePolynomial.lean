...
def readCachedMLPData : IO $ Array MLPData := do
  ...
  | .ok cachedJson => match jsonToArrayMLP cachedJson with
    | .error e => panic! e
    | .ok cache => cache.shuffle (some 42) -- shuffling will mitigate unbalanced trees

initialize cachedMLPData : Array MLPData ← readCachedMLPData

/-- The actual cached polynomials -/
def multilinearLagrangePolynomialCache :
    Std.RBMap (Nat × Nat) (MultilinearPolynomial $ Zmod n) compare :=
  cachedMLPData.foldl (fun acc (k, v) => acc.insert k (.ofList v _)) default
